...
Neural ODE batch training for the Southern Region. 
...
S = ["AL", "AR", "DE", "FL", "GA", "KY", "LA", "MD", "NC", "OK", "SC", "TN", "TX", "VA"]

tstart = 0.0
tend = 20.0
sampling = 1

model_params_s = [sqrt(0.3), sqrt(0.9), sqrt(0.19), sqrt(0.5), sqrt(0.01159)]
...
function predict_adjoint(batch)
    prob=ODEProblem(model, batch[:, 1],(tstart,tend), model_params_s)
    sol = Array(concrete_solve(prob, Tsit5(), batch[:, 1], model_params_s, saveat=tstart:sampling:tend, 
...
global cat_s = Array{Float64}(undef, 0, 6)
for i in 1:length(S)
    data = readdlm("tuples/yearly_tuples_prop_" * S[i] * ".csv", ',', Float64)
    global cat_s = vcat(cat_s, data)   
end

train_loader_s = Flux.Data.DataLoader(cat_s', batchsize = 21)

for x in train_loader_s
    @assert size(x) == (6, 21)
    x = x'
end
...
cb() = begin
    ...
    push!(losses_s, mean(ls))
end

ps_s = Flux.params(model_params_s)
opt=ADAM(0.001)

Flux.train!(loss_adjoint, ps_s, ncycle(train_loader_s, 10000), opt, cb = cb)
println("Fitted parameters:")
println("$((model_params_s.^2))")

avg_losses_s = []
for i in 1:length(losses_s)
    if i%length(S) == 0
        push!(avg_losses_s, losses_s[i])
    end
end

ls = plot(avg_losses_s, xlabel = "Iterations", ylabel = "Loss", yaxis = :log, legend = false)
savefig(ls, "figs/loss_s.pdf")
display(ls)
...
function plotFit(param, u0, data, title)
    ...
    # title!(pl,title)
    savefig(pl, "figs/fitdyn_" * title * "_s.pdf")
    display(pl)
#     return(Array(sol_fit))
end

for i in 1:length(S)
    dat = readdlm("tuples/yearly_tuples_prop_" * S[i] * ".csv", ',', Float64)
    plotFit(model_params_s, dat[1,:], dat, S[i])
end