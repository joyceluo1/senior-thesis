...
# data = readdlm("yearly_tuples_prop_CA.csv", ',', Float64)
# Q = ifelse.(data .> 0, 1, data)

# try taking the log of the prediction and the data in the loss
# try normalizing (divide by data), need an if else statement
# loss would be average of the losses across states 
...
#         for j in 1:6
#             if (data[i, j] != 0) 
#                 part = (abs(prediction_t[i, j] - data[i, j]))/data[i, j]
#                 push!(parts, part)
#             end 
#         end
#     end
...
# function plotFit(param, u0)

#     tspan=(tstart,tend)
#     sol_fit=solve(ODEProblem(model,u0, tspan, param), Tsit5(), saveat=tstart:sampling:tend)

# #     tgrid=tstart:sampling:tend
#     pl=plot(sol_fit, vars=[2 3 4 5 6], lw=2, ylim=(0, 0.03), legend=:outertopright, label = ["P" "I" "A" "R" "D"])
...
#     xlabel!(pl,"Time")
#     ylabel!(pl,"Population Prop")
# #     title!(pl,"Model Parameter Fits")
# #     savefig("restr_fitting_exp_behav.png")
#     display(pl)
...
for i in 1:length(states)
    ...
    #     init = [0.0100586, 0.0014296, 0.0052980, 0.0005185, 3.6959E-05]
